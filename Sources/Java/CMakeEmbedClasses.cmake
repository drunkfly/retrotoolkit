    ...
    if("${className}" MATCHES "^drunkfly/")
        ...
        foreach(it ${jniOutput})
            ...
            if("${what}" STREQUAL "field")
                ...
                set(cod1 "${cod1}            resolveField(${name}ID, \"${name}\", \"${desc}\");\n")
                set(cod2 "${cod2}        jfieldID ${name}ID = 0;\n")

                set(cod3 "${cod3}\n")
                set(cod3 "${cod3}        ${type} get${capitalizedName}(jobject self) const\n")
                set(cod3 "${cod3}        {\n")
                set(cod3 "${cod3}            assert(${name}ID);\n")
                set(cod3 "${cod3}            if (!${name}ID)\n")
                set(cod3 "${cod3}                throw CompilerError(nullptr, \"Internal compiler error.\");\n")
                set(cod3 "${cod3}            auto env = JVM::jniEnv();\n")
                set(cod3 "${cod3}            return env->vtbl->Get${meth}Field(env, self, ${name}ID);\n")
                set(cod3 "${cod3}        }\n")
                set(cod3 "${cod3}\n")
                ...
                set(cod3 "${cod3}                throw CompilerError(nullptr, \"Internal compiler error.\");\n")
                set(cod3 "${cod3}            auto env = JVM::jniEnv();\n")
                set(cod3 "${cod3}            env->vtbl->Set${meth}Field(env, self, ${name}ID, value);\n")
                set(cod3 "${cod3}        }\n")

                set(cod4 "${cod4}            ${name}ID = 0;\n")
                set(release TRUE)
            elseif("${what}" STREQUAL "method")
                ...
                if("${name}" STREQUAL "<init>")
                    set(cod1 "${cod1}            resolveMethod(constructorID, \"<init>\", \"${desc}\");\n")
                    set(cod2 "${cod2}        jmethodID constructorID = 0;\n")

                    set(cod3 "${cod3}\n")
                    set(cod3 "${cod3}        jobject construct(${args}) const\n")
                    set(cod3 "${cod3}        {\n")
                    set(cod3 "${cod3}            assert(constructorID);\n")
                    set(cod3 "${cod3}            if (!constructorID)\n")
                    set(cod3 "${cod3}                throw CompilerError(nullptr, \"Internal compiler error.\");\n")
                    set(cod3 "${cod3}            auto env = JVM::jniEnv();\n")
                    set(cod3 "${cod3}            return env->vtbl->NewObject(env, toJNI(), constructorID${call});\n")
                    set(cod3 "${cod3}        }\n")

                    set(cod4 "${cod4}            constructorID = 0;\n")
                    set(release TRUE)
                else()
                    set(cod1 "${cod1}            resolveMethod(${name}ID, \"${name}\", \"${desc}\");\n")
                    set(cod2 "${cod2}        jmethodID ${name}ID = 0;\n")
                    ...
                    if("${type}" STREQUAL "void")
                        set(cod3 "${cod3}           env->vtbl->CallVoidMethod(env, self, ${name}ID${call});\n")
                    else()
                        set(cod3 "${cod3}           return env->vtbl->Call${meth}Method(env, self, ${name}ID${call});\n")
                    endif()
                    set(cod3 "${cod3}        }\n")
